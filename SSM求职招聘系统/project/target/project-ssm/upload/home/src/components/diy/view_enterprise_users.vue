		...
		<el-row class="row_ce">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="企业编号" prop="enterprise_id">
												<el-input id="enterprise_id" v-model="form_sub['enterprise_id']" placeholder="请输入企业编号"
							  v-if="user_group === '管理员' || (form_sub['enterprise_users_id'] && $check_field('set','enterprise_id')) || (!form_sub['enterprise_users_id'] && $check_field('add','enterprise_id'))"></el-input>
					<div v-else-if="$check_field('get','enterprise_id')">{{form_sub['enterprise_id']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="企业名称" prop="enterprise_name">
							  ...
							  v-if="user_group === '管理员' || (form_sub['enterprise_users_id'] && $check_field('set','enterprise_name')) || (!form_sub['enterprise_users_id'] && $check_field('add','enterprise_name'))"></el-input>
				...
				<el-form-item label="人事姓名" prop="personnel_name">
												<el-input id="personnel_name" v-model="form_sub['personnel_name']" placeholder="请输入人事姓名"
							  v-if="user_group === '管理员' || (form_sub['enterprise_users_id'] && $check_field('set','personnel_name')) || (!form_sub['enterprise_users_id'] && $check_field('add','personnel_name'))"></el-input>
				...
				<el-form-item label="企业电话" prop="enterprise_phone_number">
												<el-input id="enterprise_phone_number" v-model="form_sub['enterprise_phone_number']" placeholder="请输入企业电话"
							  v-if="user_group === '管理员' || (form_sub['enterprise_users_id'] && $check_field('set','enterprise_phone_number')) || (!form_sub['enterprise_users_id'] && $check_field('add','enterprise_phone_number'))"></el-input>
					<div v-else-if="$check_field('get','enterprise_phone_number')">{{form_sub['enterprise_phone_number']}}</div>
											</el-form-item>
			</el-col>
		...
		methods: {
			...
			async submit_check(param){
				...
				var form_sub = Object.assign({} ,this.form_sub);
									if(!ret && !form_sub["enterprise_id"]){
					ret = "企业编号不能为空!";
				} else if (!ret && !form_sub["enterprise_users_id"]) {
					let res = await this.$get("~/api/enterprise_users/count?", {"enterprise_id": form_sub["enterprise_id"]});
					if(res.result){
						ret = "企业编号已存在!";
					}
				}
