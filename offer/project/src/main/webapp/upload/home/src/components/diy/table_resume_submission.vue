    ...
    <el-form label-position="right" :model="query" class="form p_4" label-width="120">
      ...
      <el-row class="rows row2">
         ...
         <el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
            ...
            <el-button v-if="user_group == '管理员' || $check_action('/resume_submission/table','add') || $check_action('/resume_submission/view','add')" @click="$router.push('./view?')" class="add">添加</el-button>
						
            <el-button v-if="user_group == '管理员' || $check_action('/resume_submission/table','del') || $check_action('/resume_submission/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
                        </el-col>
                ...
                <el-table-column fixed type="selection" tooltip-effect="dark" width="55">
                      ...
                      {{ get_user_enterprise_users(scope.row['enterprise_users']) }}
                               ...
                               v-if="user_group == '管理员' || $check_field('get','work_city')" min-width="200">
                                              ...
                                              <el-table-column prop="resume_file" @sort-change="$sortChange" label="简历文件"
                               v-if="user_group == '管理员' || $check_field('get','resume_file')" min-width="200">
                                      <template slot-scope="scope">
                      <a :href="$fullUrl(scope.row['resume_file'])" target="_blank" style="color: rgb(64, 158, 255);">点击下载</a>
                    </template>
                                </el-table-column>
      ...
      <el-table-column sortable prop="update_time" label="更新时间" min-width="200">
        <template slot-scope="scope">
          {{ $toTime(scope.row["update_time"],"yyyy-MM-dd hh:mm:ss") }}
                ...
                <el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/resume_submission/table','set') || $check_action('/resume_submission/view','set') || $check_action('/resume_submission/view','get') || $check_action('/通知/table','add') || $check_action('/通知/view','add')" >
        

      <template slot-scope="scope">
        <router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
                     v-if="user_group == '管理员' || $check_action('/resume_submission/table','set') || $check_action('/resume_submission/view','set') || $check_action('/resume_submission/view','get')"
                                 :to="'./view?' + field + '=' + scope.row[field]"
                                 size="small">
          <span>详情</span>
        </router-link>
                                        <!--跨表按钮-->
                                      <el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/interview_notification/view')" v-if="user_group == '管理员' || $check_action('/interview_notification/table','add') || $check_action('/interview_notification/view','add')">
                      <span>通知</span>
                    </el-button>
                                                                                        </template>
    </el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        showModal: false,
        // 获取数据地址
        url_get_list: "~/api/resume_submission/get_list?like=0",
        url_del: "~/api/resume_submission/del?",
