<template>
	<div>
		<b-form-group v-if="$check_register_field('add','full_name','/user_registration/view')" id="input-group-0" label="姓名" label-for="input-0">
		...
		<b-form-group v-if="$check_register_field('add','date_of_birth','/user_registration/view')" id="input-group-2" label="出生日期" label-for="input-2">
			<b-form-input id="input-2" v-model="form['date_of_birth']" type="date" placeholder="出生日期" trim></b-form-input>
		</b-form-group>
	</div>
</template>
