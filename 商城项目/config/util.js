...
const translateGroupBuyStatus = (groupBuyStatus) => {
	...
	let groupBuyStatusColor = '#ffbd71';
...
const translateOrderAfterSaleStatus = (status) => {
	let statusDesc = '';
	let statusColor = '#ffbd71';
	switch (status) {
		case '0':
			{
			statusDesc = '退款中';
...
const translateWithdrawStatus = (withdrawStatus) => {
	...
	let withdrawStatusColor = '#ffbd71';
...
//设置商品列表在购物车的数量
const prodCartNum = (productList,cartList) => {
	let fList = [];
	if(productList){
		productList.forEach(function(product, pIndex){
			let cartNum = 0;
			if(cartList)
				cartList.forEach(function(cart, cIndex){
					if(product.productUuid == cart.productDTO.productUuid){
						cartNum = cartNum + cart.unit;
					}
				})
			product.cartNum = cartNum;
			fList.push(product);
		})
	}
	return fList;
...
//设置单个商品在购物车的数量
const singleProdCartNum = (product,cartList) => {
	let cartNum = 0;
	if(cartList)
		cartList.forEach(function(cart, cIndex){
			if(product.productUuid == cart.productDTO.productUuid){
