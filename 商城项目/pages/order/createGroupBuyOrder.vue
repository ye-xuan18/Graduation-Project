<template>
	<view>
		...
		<view class="navbar" v-if="navList.length>0">
			<view v-for="(item, index) in navList" :key="index" class="nav-item" :class="{current: tabCurrentIndex === index}" @click="tabClick(index,item)">
				{{item.text}}
			</view>
		</view>
		...
		methods: {
			...
			populateDeliveryType(){
				...
				if(this.isDeliveryPickEnabled){
					this.navList.push({state:'2',text:'门店自提'});
				}
			...
			submit() {
				...
				if (this.currentDeliveryType=='2'&&!this.merchantData.merchantUuid) {
