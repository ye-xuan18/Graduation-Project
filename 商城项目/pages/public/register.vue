<template>
	<view class="container">
		...
		<view class="wrapper">
			...
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN -->
			<button class="confirm-btn" @click="getuserinfo" :disabled="registering">微信登录</button>
			<!-- #endif -->
	...
	export default {
			...
			if(this.isWexinBrowser()){
				...
				if(!openId){
					...
					//没有code, 重定向获取code
						...
						let appIdOfficialAccount = this.applicationConfig.wechatAppIdOfficialAccount;
						let authURL = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='+appIdOfficialAccount+'&redirect_uri='+uri+'&response_type=code&scope=snsapi_base&state=123#wechat_redirect';  
		...
		methods: {
			...
			async toRegisterByPassword() {
				...
				if (this.supervisorId && this.supervisorId != 'undefined') {
					requestData.supervisorL1 = {
