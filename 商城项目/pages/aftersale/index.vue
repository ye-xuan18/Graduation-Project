	...
	<view class="content">
		...
		<view class="row b-b">
			...
			<radio-group class="input" @change="radioChange">
				<label>
					<radio value="1" :checked="afterSaleType=='1'"/><text>仅退款</text>
				</label>
				<!-- 待发货状态只需要退款即可 -->
				<label v-if="order.orderStatus != '1'">
					<radio value="2" :checked="afterSaleType=='2'"/><text>退款退货</text>
				</label>
			</radio-group>
		</view>
		<view class="row">
		...
		<view class="aftersale-image">
			<text>上传凭证</text>
			<custom-image-upload v-model="imageUrlList" fileKeyName="files" :header="header" :formData="formData" :server-url="uploadUrl"></custom-image-upload>
		</view>
...
<script>
	...
	import customImageUpload from '../../components/custom-image-upload/custom-image-upload.vue'
	export default {
		components: {customImageUpload},
		data() {
			return {
