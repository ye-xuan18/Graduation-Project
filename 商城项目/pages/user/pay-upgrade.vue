<template>
	<view class="content">
		<view class="price-box">
			<text>支付金额</text>
			<text class="price">{{userUpgrade.transactionAmount}}</text>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.inquiryUserUpgrade(this.tranCode);
		...
		methods: {
			...
			wechatPayH5() {
				...
				that.$api.request.upgradeWechatPayH5(options, res => {
					if (res.body.status.statusCode == 0) {
						var data = res.body.data;
					...
					if (res.body.status.statusCode == 0) {
						...
						wx.ready(function() {
							wx.chooseWXPay({
								...
								success: function(res) {
									uni.redirectTo({
										url: '/pages/user/pay-upgrade-success'
									})
								}
							});
						})
			...
			wechatPayMP() {
				...
				uni.login({
					...
					success: function(loginRes) {
						...
						that.$api.request.upgradeWechatPay({
							transactionCode: that.tranCode,
							code: loginRes.code
						}, res => {
							if (res.body.status.statusCode === '0') {
			...
			//支付宝在H5中支付
			alipayH5() {
				this.$api.request.upgradeAlipay({
			...
			//支付宝在APP中支付
			alipayApp() {
				this.$api.request.upgradeAlipayApp({
					transactionCode: this.tranCode
				}, res => {
					if (res.body.status.statusCode === '0') {
						let alipayForm = res.body.data.alipayForm;
			...
			//余额支付
			balancePay() {
				this.$api.request.upgradeBalancePay({
					transactionCode: this.tranCode,
					paymentMethod: this.payType
				}, res => {
					if (res.body.status.statusCode === '0') {
			...
			//查询会员购买详情
			inquiryUserUpgrade(tranCode) {
				this.$api.request.inquiryUserUpgrade({
					transactionCode: tranCode
				}, res => {
					if (res.body.status.statusCode === '0') {
						this.userUpgrade = res.body.data;
					} else {
						console.log(res.body.status.errorDesc);
					}
				});
